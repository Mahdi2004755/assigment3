...
def showGbp():
    """
    This function sends GET request to CoinDesk API and shows Bitcoin's current price in GBP.
    """
    response = requests.get(url)  # sending request to url
    jsonResponse = response.json()  # saving response as JSON
    time = jsonResponse["time"]["updated"]  # getting time from response
...
def showEuro():
       ...
       """
    This function sends GET request to CoinDesk API and shows Bitcoin's current price in EUR.
    """
    response = requests.get(url)  # sending request to url
    jsonResponse = response.json()  # saving response as JSON
    time = jsonResponse["time"]["updated"]  # getting time from response
